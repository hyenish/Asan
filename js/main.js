const elFooter = document.querySelector('footer'); //footer부분을 가져오는 것
const elcon_wrap = document.querySelector('#con_wrap'); 

//#con_wrap의 높이 구하기 -footer에 fixed를 사용 안할 때
let winH = $(window).height(); //열려져 있는 화면의 높이값
const headerH = $('header').height(); //header의 높이값
const footerH = elFooter.offsetHeight; //footer의 높이값(자바스크립트)
//console.log(`현재 화면 높이 ${winH} / header의 높이 ${headerH} / footer의 높이 ${footerH}`); 


//coon_wrap의 높이는 화면전체 높이값에서 헤더, 푸터의 높이값을 빼준 값
let con_wrap_Height = winH - (headerH + footerH);
$('#con_wrap').css({height:con_wrap_Height}); //제이쿼리
//elcon_wrap.style.height = con_wrap_Height; //자바스크립트
...
var du = 400;
var open = 0;;  //메뉴가 닫혀있는 상태를 의미하는 변수(처음)
